...
def plot_triangulations(*points_triangulated, labels=None, idx=None):
    ...
    for i, arr in enumerate(points_triangulated):
        ...
        ax.scatter(
            ...
            arr[:, 2],
            c=colors[i],
            label=labels[i] if labels is not None else f"Triangulation {i+1}",
        )
    ax.set_xlabel("X")
    ax.set_ylabel("Y")
    ax.set_zlabel("Z")
...
def plot_back_projections(video_dir, back_projections, idx):
    ...
    camera_sequence = [
        Path(video_path).stem.split("_")[-1].split(".avi")[0]
    ...
    for i, view in enumerate(camera_sequence):
        ...
        plt.scatter(
            back_projections[view][:, 0], back_projections[view][:, 1], c="r", s=10
        )
        plt.title(view)
    plt.show()
