...
# ):
slp_file = "/Users/vigji/Downloads/labels.v001.pkg.slp"
dlc_dir = "/Users/vigji/Desktop/converted_from_sleap/test6"
project_name = "test4"
scorer = "me"
"""
...
with h5py.File(slp_file, "r") as hdf_file:
    video_keys = [k for k in hdf_file.keys() if k.startswith("video") and k[5:].isdigit()]
    video_groups = [f"video{i}" for i in range(len(video_keys))]
...
with h5py.File(slp_file, "r") as hdf_file:
    ...
    # [('instance_id', '<i8'), ('instance_type', '|u1'), ('frame_id', '<u8'), 
    ...
    for idx, frame_id in enumerate(frame_ids):
        # If this frame_id isn't in the dict yet, create an empty list for it
        if frame_id not in frame_to_instances:
            frame_to_instances[frame_id] = []
        frame_to_instances[frame_id].append(idx)

    # Read all points once
    points_dataset = hdf_file["points"][:]

np.unique(np.diff(frame_ids))
instances_dataset.shape

# %%
with h5py.File(slp_file, "r") as hdf_file:
    # Process each video group
    for video_group in video_groups[:1]:
...
@dataclass
class Args:
    slp_file: str = "/Users/vigji/Downloads/labels.v001.pkg.slp"
    dlc_dir: str = "/Users/vigji/Desktop/converted_from_sleap/test4"
    run_dlc: bool = False
