...
def generate_vector_checkerboard_pdf(
    square_size_cm, num_squares_y, num_squares_x, filename, topleft="white"
):
    ...
    for y in range(num_squares_y):
        for x in range(num_squares_x):
            if (
                x + y
            ) % 2 + off == 1:  # Fill the square if it's part of the checker pattern
                c.rect(
...
# Example usage
generate_vector_checkerboard_pdf(
    1, 8, 11, "/Users/vigji/Desktop/vector_checkerboard.pdf", topleft="white"
)
